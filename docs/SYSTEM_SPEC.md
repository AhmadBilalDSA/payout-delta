...
- **Phase I — Reverse Target-Gross Invoice Solver**
  (`components/Calculator.tsx` + `lib/calculatorEngine.ts`) — the exact-net
  "Invoice Solver (Target Net Received)" mode returns the billable USD gross
  for a target net deposit, highlights "To receive exactly {net} {ccy}, invoice
  your client for {gross} USD." and ships an emerald "Apply to Invoice Studio"
  that syncs the statutory bank/tier bench + gross-up line item through the
  `payoutdelta:invoice_sync` bridge.
- **Phase I — Embeddable Backlink Widget Engine**
  (`app/embed/[corridor]/` + `components/EmbedSnippetModal.tsx`) — 50 static
  `/embed/<slug>/` corridor cards (dynamicParams = false) whose nested layout's
  document-global `<style>` zero-JS suppresses the `header` / `market-status`
  aside / `footer` chrome; each forced-dark card shows the active mid-market
  rate, the receiving bank's SWIFT intermediary cut and the real net take-home
  on a $1,000 payout, plus a "Verified by PayoutDelta" backlink badge to the
  full calculator. The snippet modal builds a live-preview iframe embed code
  with one-click copy — publisher-friendly backlinks with zero third-party
  runtime.
...
| I | **Reverse Target-Gross Invoice Solver & Embeddable Backlink Widget Engine** — closed-form inverse solver prompting the exact net-to-bill gross in USD (with an emerald "Apply to Invoice Studio" that syncs the statutory bank/tier bench + gross-up line item to the studio); 50 static `/embed/<slug>/` backlink widgets via `generateStaticParams` + `dynamicParams = false`, whose `app/embed/layout.tsx` document-global `<style>` suppresses `header`/`footer`/`market-status` aside chrome with zero JS; `components/EmbedSnippetModal.tsx` live-preview snippet generator with 1-click copy + toast; sitemap `/embed/<slug>/` entries, llms.txt embed guide entry, and a Phase I embed audit battery in `scripts/test_corridors.mjs` (export, metadata, badge, backlink, assets, links) | **Shipped** (`c1b4548`) |
...
npm run indexnow              # OPTIONAL deploy-time — offline-safe IndexNow ping of all 204 canonical routes (warns only, never fails)
```
...
canonical route set (12 static + 50 base + 87 long-tail + 50 embed + 5
localized = 204 URLs at `https://ahmadbilaldsa.github.io/payout-delta`, matching the sitemap
trailing-slash canonicals) and POSTs it to `https://api.indexnow.org/indexnow` — offline-safe
...
- `c1b4548` — **Phase I**: Reverse Target-Gross Invoice Solver & Embeddable Backlink Widget Engine (`components/Calculator.tsx` Mode B relabeled "Invoice Solver (Target Net Received)" alongside "Standard (From USD Sent)" via the existing `modeQuoteAudit` / `modeTargetGoal` keys ×7 catalogs; exact-net solver callout "To receive exactly {net} {ccy}, invoice your client for {gross} USD." + emerald "Apply to Invoice Studio" that writes the statutory bank/tier bench + gross-up line item through the `payoutdelta:invoice_sync` bridge (`writeLocalStorage(INVOICE_SYNC_KEY)` + `payoutdelta:synced` CustomEvent, mirroring `TransactionCostingWidget.handleSync`) and navigates to `/invoice/`; `app/embed/[corridor]/` static widget engine — 50 base corridors via `generateStaticParams` + `dynamicParams = false`, `app/embed/layout.tsx` nested shell whose document-global `<style>` suppresses `header`/`footer`/`market-status` aside chrome with zero JS, forced-dark card (active mid-market rate, SWIFT BIC intermediary cut, real net take-home on $1,000, "Verified by PayoutDelta" backlink badge to the full calculator); `components/EmbedSnippetModal.tsx` live-preview snippet generator with 1-click copy + 2.5s "✓ Copied" toast; sitemap `/embed/<slug>/` entries, llms.txt embed guide entry, `scripts/test_corridors.mjs` Phase I embed battery (export, metadata, badge, backlink, assets, links), docs).